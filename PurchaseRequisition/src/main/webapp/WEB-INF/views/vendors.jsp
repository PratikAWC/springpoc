	...
	<head>
		...
		<script language="text/">
			function selAll(source) {
			  checkboxes = document.getElementsByName('chbvendor');
			...
			<form action="">
				<table border="1" style="width: 90%; border-collapse: collapse; border-color: #006400;">
					<tr style="height: 35px; text-align: center; color: white; background-color: #368BC1;">
						<td><input type="checkbox" name="chbselall" id="chbselall" onClick="selAll(this);"/>ALL</td>
						<td>NAME</td><td>TYPE</td><td>PAN NO.</td><td>REGISTERED ADDRESS</td>
					</tr>
					<c:forEach items="${category}" var="cat">
						<tr style="height: 30px; text-align: center;">
							<td>
								<input type="checkbox" name="chbvendor" id="chbvendor" />
							</td>
							<td>${cat.name}</td> 
							<td>${cat.type}</td> 
							<td>${cat.pan}</td> 
