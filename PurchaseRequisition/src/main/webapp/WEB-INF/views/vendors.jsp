	...
	<body><br/> 
		<form action="./sendNotification" method="post">		
			...
			<table border="1" style="width: 90%; border-collapse: collapse; border-color: #006400;">
				...
				<c:forEach items="${category}" var="cat">
					<tr style="height: 30px; text-align: center;">
						<td>
							<input type="checkbox" class="checkBoxClass" name="pan" id="pan" value="${cat.pan}" />
						</td>
						<td>${cat.pan}</td> 
						<td>${cat.name}</td> 
						<td>${cat.type}</td> 
			...
			<div style="margin: 0 auto;">
				<input type="submit" name="btnvendor" id="btnvendor" style="background-color: #368BC1; color: white; border-radius: 5px; font-size:18px; width: 150px; height: 45px; border-bottom-right-radius: 15px; border-top-left-radius: 15px;" value="Notify" />
			</div>
		</center>
		</form>
	</body>
